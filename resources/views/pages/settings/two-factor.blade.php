...
new class extends Component {
    ...
    public string $twoFactorType = 'totp';
    ...
    public function mount(DisableTwoFactorAuthentication $disableTwoFactorAuthentication): void
    {
        ...
        $this->twoFactorType = auth()->user()->two_factor_type ?? 'totp';
    ...
    public function enable(EnableTwoFactorAuthentication $enableTwoFactorAuthentication): void
    {
        ...
        auth()->user()->forceFill(['two_factor_type' => 'totp'])->save();
    ...
    public function enableEmail(): void
    {
        ...
        $user->forceFill([
            'two_factor_secret' => encrypt('email-marker'),
    ...
    private function sendEmailCode($user)
    {
        $code = (string) rand(100000, 999999);
        \Illuminate\Support\Facades\Cache::put('two_factor_code_' . $user->id, $code, now()->addMinutes(10));
    ...
    public function confirmTwoFactor(ConfirmTwoFactorAuthentication $confirmTwoFactorAuthentication): void
    {
        ...
        if ($user->two_factor_type === 'email') {
            $cachedCode = \Illuminate\Support\Facades\Cache::get('two_factor_code_' . $user->id);
            ...
            \Illuminate\Support\Facades\Cache::forget('two_factor_code_' . $user->id);
        } else {
                ...
                <div class="space-y-4">
                    ...
                    <flux:text>
                        @if($twoFactorType === 'email')
                            {{ __('With two-factor authentication enabled, you will be prompted for a secure code sent to your email during login.') }}
                        @else
            ...
            @else
                <div class="space-y-4">
                    ...
                    <flux:text variant="subtle">
                        {{ __('Choose your preferred method for two-factor authentication.') }}
                    </flux:text>

                    <div class="flex flex-col gap-4 sm:flex-row">
                        <!-- Authenticator App -->
                        <div
                            class="flex-1 p-4 border rounded-xl border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800/50">
                            <flux:heading size="lg" class="mb-2">{{ __('Authenticator App') }}</flux:heading>
                            <flux:text class="mb-4 text-sm">
                                {{ __('Use Google Authenticator, Authy, or similar apps to generate codes.') }}
                            </flux:text>
                            <flux:button variant="primary" icon="device-phone-mobile" class="w-full" wire:click="enable">
                                {{ __('Enable App 2FA') }}
                            </flux:button>
                        </div>
                        ...
                        <div
                            class="flex-1 p-4 border rounded-xl border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800/50">
                            <flux:heading size="lg" class="mb-2">{{ __('Email') }}</flux:heading>
                            <flux:text class="mb-4 text-sm">
                                {{ __('Receive security codes via your registered email address.') }}
                            </flux:text>
                            <flux:button variant="filled" icon="envelope" class="w-full" wire:click="enableEmail">
                                {{ __('Enable Email 2FA') }}
                            </flux:button>
                        </div>
                    </div>
                ...
                @if($this->twoFactorType === 'email')
                    <div class="space-y-4 text-center">
                        <div class="p-4 rounded-lg bg-stone-50 dark:bg-stone-800">
                            ...
                            <flux:text>{{ __('A verification code has been sent to your email address.') }}</flux:text>
                        </div>

                        <div>
                            <flux:button variant="primary" class="w-full" wire:click="showVerificationIfNecessary">
                                {{ __('Enter Code') }}
                            </flux:button>
                        </div>
                    </div>
