...
new #[Title('Edit Character')] class extends Component {
    ...
    public function rules()
    {
        return [
            ...
            'alignment' => ['nullable', 'string', 'max:255'], // Alignment is not an Enum yet (it's hardcoded in the view)
    ...
    public function mount(Character $character)
    {
        ...
        $this->status = $character->status ?? 'Active';
        $this->notes = $character->notes;
        $this->classes = $character->classes ?? [['class' => '', 'level' => 1]];
        $this->max_hp = $character->max_hp;
        $this->current_hp = $character->current_hp;
        $this->current_xp = $character->current_xp;
        ...
        $this->relationships = $character->relationships ?? [];

        // Convert inventory object to array if needed to map over inputs
        $this->inventory = [];
        if (!empty($character->inventory)) {
            foreach ($character->inventory as $key => $item) {
                // Ensure it's format-compatible with the form
                $this->inventory[] = [
                    'name' => $item['name'] ?? $key,
    ...
    public function save()
    {
        ...
        $this->dispatch(
            'notify',
            title: __('Lenda Atualizada'),
            message: __('Alterações salvas com sucesso!'),
            type: 'success'
        );

        return $this->redirectRoute('characters.show', ['character' => $this->character->id], navigate: true);
    }
};
?>
<div class="flex h-full w-full flex-1 flex-col gap-8 rounded-xl max-w-4xl mx-auto px-4 py-8">
    <div>
        <flux:breadcrumbs class="mb-4">
            ...
            <flux:breadcrumbs.item href="{{ route('characters.show', $character) }}" wire:navigate>
                {{ $character->name }}
            </flux:breadcrumbs.item>
            <flux:breadcrumbs.item>{{ __('Edit') }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>

        <h1 class="text-3xl font-black text-stone-900 dark:text-white mt-4 mb-2">{{ __('Editar Personagem') }}</h1>
        <p class="text-stone-600 dark:text-stone-400 font-medium">{{ __('Atualize a lenda do seu herói.') }}</p>
    </div>
    ...
    <form wire:submit="save"
        ...
        <div class="flex justify-end gap-3 pt-6 border-t border-stone-200 dark:border-stone-800">
            <flux:button href="{{ route('characters.show', $character) }}" variant="ghost" wire:navigate>
                {{ __('Cancel') }}
            </flux:button>
